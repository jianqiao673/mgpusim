...
type GPUOperator struct {
	...
	embeddingForwardKernel              *insts.HsaCo
    embeddingBackwardWeightKernel       *insts.HsaCo
	attentionKernel                     *insts.HsaCo
...
func (o *GPUOperator) loadKernels() {
	...
	loadKernel(&o.embeddingForwardKernel, embeddingKernelBytes, "embedding_forward")
    loadKernel(&o.embeddingBackwardWeightKernel, embeddingKernelBytes, "embedding_backward_weight")
	loadKernel(&o.attentionKernel, attentionKernelBytes, "causal_self_attention")
...
func (o *GPUOperator) Free(t tensor.Tensor) {
	fmt.Printf("[Free tensor] ptr: 0x%x, size: %d\n", t.(*Tensor).ptr, t.NumElement()*sizeOfFloat32)
...
// EmbeddingForwardArgs 定义 embedding 前向传播参数
type EmbeddingForwardArgs struct {
    InputIndices        driver.Ptr
    Weight              driver.Ptr
    Output              driver.Ptr
    VocabSize           int32
    EmbeddingDim        int32
...
// EmbeddingBackwardWeightArgs 定义 embedding 权重梯度计算参数
type EmbeddingBackwardWeightArgs struct {
    InputIndices        driver.Ptr
    GradOutput          driver.Ptr
    GradWeight          driver.Ptr
    VocabSize           int32
    EmbeddingDim        int32
    BatchSize           int32
    SeqLen              int32
...
// CreateEmbeddingOutputTensor 创建 embedding 前向传播的输出张量
func (o *GPUOperator) CreateEmbeddingOutputTensor(inputIndices, weight tensor.Tensor) tensor.Tensor {
    batchSize := inputIndices.Size()[0]
    seqLen := inputIndices.Size()[1]
    embeddingDim := weight.Size()[1]
    
    return o.Create([]int{batchSize, seqLen, embeddingDim})
}

// LazyCreateEmbeddingOutputTensor 内存优化版本的创建 embedding 输出张量
func (o *GPUOperator) LazyCreateEmbeddingOutputTensor(inputIndices, weight tensor.Tensor) tensor.Tensor {
    batchSize := inputIndices.Size()[0]
    seqLen := inputIndices.Size()[1]
    embeddingDim := weight.Size()[1]
    
	return o.PureCreate([]int{batchSize, seqLen, embeddingDim})
}

// EmbeddingForward 执行 embedding 前向传播（修正版）
func (o *GPUOperator) EmbeddingForward(
    inputIndices, weight tensor.Tensor,
    paddingIdx int,
) tensor.Tensor {
    var cpuOutput tensor.Tensor
    if o.verification {
        cpuInputIndices := o.gpuTensorToCPUTensor(inputIndices)
        cpuWeight := o.gpuTensorToCPUTensor(weight)
        cpuOutput = o.cpuOperator.EmbeddingForward(cpuInputIndices, cpuWeight, paddingIdx)
    }

    batchSize := inputIndices.Size()[0]
    seqLen := inputIndices.Size()[1]
    embeddingDim := weight.Size()[1]
    output := o.Create([]int{batchSize, seqLen, embeddingDim})

    inputDevice := inputIndices.(*Tensor)
    weightDevice := weight.(*Tensor)
    outputDevice := output.(*Tensor)

    totalElements := batchSize * seqLen

    args := &EmbeddingForwardArgs{
        InputIndices: inputDevice.ptr,
        Weight:       weightDevice.ptr,
        Output:       outputDevice.ptr,
        VocabSize:    int32(weight.Size()[0]),
        EmbeddingDim: int32(embeddingDim),
        BatchSize:    int32(batchSize),
        SeqLen:       int32(seqLen),
        PaddingIdx:   int32(paddingIdx),
    }

    o.timerStart()
    o.driver.LaunchKernel(
        o.ctx,
        o.embeddingForwardKernel,
        [3]uint32{uint32(totalElements), 1, 1},
        [3]uint16{256, 1, 1},
        args,
    )
    o.timerEnd("EmbeddingForward")

    if o.verification {
        o.tensorMustMatch(cpuOutput, output)
        o.cpuOperator.Free(cpuOutput)
    }
...
func (o *GPUOperator) EmbeddingBackwardWeight(
	inputIndices, gradOutput, weight tensor.Tensor,
	paddingIdx int,
	scale float32,
) tensor.Tensor {
	var cpuGradWeight tensor.Tensor
	if o.verification {
		cpuInputIndices := o.gpuTensorToCPUTensor(inputIndices)
		cpuGradOutput := o.gpuTensorToCPUTensor(gradOutput)
		cpuWeight := o.gpuTensorToCPUTensor(weight) // 传入权重
		cpuGradWeight = o.cpuOperator.EmbeddingBackwardWeight(cpuInputIndices, cpuGradOutput, cpuWeight, paddingIdx, scale)
	}

	vocabSize := gradOutput.Size()[0]
	embeddingDim := gradOutput.Size()[2]
	gradWeight := o.Create([]int{vocabSize, embeddingDim})

	inputDevice := inputIndices.(*Tensor)
	gradOutputDevice := gradOutput.(*Tensor)
	gradWeightDevice := gradWeight.(*Tensor)

	args := &EmbeddingBackwardWeightArgs{
		InputIndices: inputDevice.ptr,
		GradOutput:   gradOutputDevice.ptr,
		GradWeight:   gradWeightDevice.ptr,
		VocabSize:    int32(vocabSize),
		EmbeddingDim: int32(embeddingDim),
		BatchSize:    int32(inputIndices.Size()[0]),
		SeqLen:       int32(inputIndices.Size()[1]),
		PaddingIdx:   int32(paddingIdx),
		Scale:        scale,
	}
	...
	o.driver.LaunchKernel(
		o.ctx,
		o.embeddingBackwardWeightKernel,
		[3]uint32{uint32(vocabSize), 1, 1},
		[3]uint16{64, 1, 1},
		args,
	)
	o.timerEnd("EmbeddingBackwardWeight")

	if o.verification {
		o.tensorMustMatch(cpuGradWeight, gradWeight)
		o.cpuOperator.Free(cpuGradWeight)
	}

	return gradWeight
}
// CausalSelfAttention performs a GPT-style causal self-attention operation.
// Input: X [B, T, C]
// Output: Y [B, T, C]
func (o *GPUOperator) CausalSelfAttention(
    x tensor.Tensor,        // [B, T, C]
    c_attn, c_proj tensor.Tensor, // [C, 3*C] 和 [C, C]
    bias tensor.Tensor,     // [1, 1, block_size, block_size]
    nHead int,
) tensor.Tensor {
    fmt.Println("[GPUOperator] Running CausalSelfAttention")
    ...
    // 1. Q, K, V = XWq, XWk, XWv
    qkv := o.Gemm(false, false, 1.0, 0.0, x, c_attn, o.Zeros([]int{B, T, 3*C}))
    
    // 2. 分割 Q, K, V
    q := o.Slice(qkv, 0, B*T*C)
    k := o.Slice(qkv, B*T*C, 2*B*T*C)  
    v := o.Slice(qkv, 2*B*T*C, 3*B*T*C)

    // 重塑为 [B, T, C]
    q = o.Reshape(q, []int{B, T, C})
    k = o.Reshape(k, []int{B, T, C})
    v = o.Reshape(v, []int{B, T, C})
    
    // 3. 多头处理
    headSize := C / nHead

    // q: [B, T, C] -> [B, T, nHead, headSize] -> [B, nHead, T, headSize]
    q = o.Reshape(q, []int{B, T, nHead, headSize})
    q = o.Transpose(q, []int{0, 2, 1, 3}) // [B, nHead, T, headSize]

    k = o.Reshape(k, []int{B, T, nHead, headSize})
    k = o.Transpose(k, []int{0, 2, 1, 3}) // [B, nHead, T, headSize]

    v = o.Reshape(v, []int{B, T, nHead, headSize}) 
    v = o.Transpose(v, []int{0, 2, 1, 3}) // [B, nHead, T, headSize]

    // 4. 计算注意力分数: (q @ k^T) / sqrt(headSize)
    kt := o.Transpose(k, []int{0, 1, 3, 2}) // [B, nHead, headSize, T]
    att := o.Gemm(false, false, 1.0/math.Sqrt(float64(headSize)), 0.0, 
                q, kt, o.Zeros([]int{B, nHead, T, T}))

    // 5. 应用因果掩码
    att = o.ApplyCausalMask(att, bias, T)

    // 6. Softmax
    att = o.Softmax(att)

    // 7. 注意力输出: att @ v
    y := o.Gemm(false, false, 1.0, 0.0, att, v, o.Zeros([]int{B, nHead, T, headSize}))

    // 8. 转置并重塑回 [B, T, C]
    y = o.Transpose(y, []int{0, 2, 1, 3}) // [B, T, nHead, headSize]
    y = o.Reshape(y, []int{B, T, C})      // [B, T, C]

    // 9. 输出投影
    y = o.Gemm(false, false, 1.0, 0.0, y, c_proj, o.Zeros([]int{B, T, C}))
...
// ApplyCausalMask 修复版本 - 使用现有操作实现masked_fill
func (o *GPUOperator) ApplyCausalMask(att, bias tensor.Tensor, T int) tensor.Tensor {
    attSize := att.Size()
    B := attSize[0]
    nHead := attSize[1]
    
    // 1. 获取掩码并扩展到正确维度
    mask := o.Slice(bias, 0, T*T)
    mask = o.Reshape(mask, []int{1, 1, T, T})
    
    // 扩展到batch和head维度
    if B > 1 || nHead > 1 {
        expandedMask := o.Repeat(mask, B*nHead)
        defer o.Free(expandedMask)
        mask = o.Reshape(expandedMask, attSize)
    }
    defer o.Free(mask)

    // 2. 创建填充值tensor (-1e9用于softmax后变成0)
    fillData := make([]float64, att.NumElement())
    for i := range fillData {
        fillData[i] = -1e9
    }
    fillTensor := o.CreateWithData(fillData, attSize, "fill_value")
    defer o.Free(fillTensor)

    // 3. 使用现有操作实现masked_fill:
    // output = mask * att + (1 - mask) * fill_value
    
    // 3.1 mask * att (保留不需要mask的位置)
    maskedAtt := o.ElementWiseMul(mask, att)
    defer o.Free(maskedAtt)
    
    // 3.2 创建全1tensor
    onesData := make([]float64, mask.NumElement())
    for i := range onesData {
        onesData[i] = 1.0
    }
    ones := o.CreateWithData(onesData, mask.Size(), "ones")
    defer o.Free(ones)
    
    // 3.3 1 - mask (反转掩码)
    invertedMask := o.ScaleAdd(1.0, -1.0, ones, mask)
    defer o.Free(invertedMask)
    
    // 3.4 (1 - mask) * fill_value (在需要mask的位置填充-1e9)
    maskedFill := o.ElementWiseMul(invertedMask, fillTensor)
    defer o.Free(maskedFill)
    
    // 3.5 最终结果: mask * att + (1 - mask) * fill_value
    output := o.ScaleAdd(1.0, 1.0, maskedAtt, maskedFill)

    if o.verification {
        o.verifyCausalMask(att, output, bias, T)
    }
...
// 验证函数
func (o *GPUOperator) verifyCausalMask(att, output, bias tensor.Tensor, T int) {
    cpuAtt := o.gpuTensorToCPUTensor(att)
    cpuOutput := o.gpuTensorToCPUTensor(output)
    cpuBias := o.gpuTensorToCPUTensor(bias)
    
    attData := cpuAtt.Vector()
    outputData := cpuOutput.Vector()
    biasData := cpuBias.Vector()
            ...
            for i := 0; i < T; i++ {
                for j := 0; j < T; j++ {
                    ...
                    if biasData[maskIdx] == 0.0 { // 需要mask的位置
                        if outputData[idx] != -1e9 {
                            fmt.Printf("Mask verification failed at [%d,%d,%d,%d]: expected -1e9, got %f\n",
                                b, h, i, j, outputData[idx])
                            valid = false
                        }
                    } else { // 不需要mask的位置
                        if math.Abs(outputData[idx]-attData[idx]) > 1e-5 {
                            fmt.Printf("Mask verification failed at [%d,%d,%d,%d]: expected %f, got %f\n",
                                b, h, i, j, attData[idx], outputData[idx])
                            valid = false
                        }
                    }
                }
    ...
    if !valid {
        panic("causal mask verification failed")
    }
    fmt.Println("ApplyCausalMask verified.")
}

func (o *GPUOperator) CreateCausalMask(blockSize int) tensor.Tensor {
    // 创建因果掩码，在初始化时调用
    ...
    return o.CreateWithData(data, []int{1, 1, blockSize, blockSize}, "causal_mask")
}